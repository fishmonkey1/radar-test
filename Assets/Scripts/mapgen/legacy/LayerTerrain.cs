...
public class LayerTerrain : MonoBehaviour
{
    ...
    public void CreateTopoTexture()
    {
        ...
        //rotate 180 deg cuz fuck if I know it doesn't line up lol
        topoObject.transform.Rotate(0, 180, 0);


        // Creating colormap
        // TODO: step the Lerp for bands
        Color[] colorMap = new Color[X * Y];
        for (int y = 0; y < Y; y++)
        {
            for (int x = 0; x < X; x++)
            {
                float elevation = finalMap.GetTile(y, x).ValuesHere[LayersEnum.Elevation];
                colorMap[x * X + y] = Color.Lerp(topoColor1, topoColor2, elevation);
            }
        }
